#!/usr/bin/env python3
"""Pinned R0-versus-R1 LongMemEval E6 development diagnostic.
...
``R1``
    ...
    family heads with the frozen E6 RRF contract, and hydrate only canonical
    raw turns into the reader prompt.
...
bound by digest.  The first decision is context-only.  Reader and development
judge calls are permitted only if R1 clears zero-margin gold-context
noninferiority and is not Pareto-dominated by R0.  GPT-4o is intentionally not
called here, and no output is eligible for composition or serving promotion.
"""
...
E6_RUN_PROTOCOL_VERSION = "swarmbrain-longmemeval-e6-r0-r1-development-v2"
...
DEFAULT_E6_OUTPUT = Path("/private/tmp/swarmbrain-longmemeval-e6-pilot-v2")
...
JUDGE_MAX_TOKENS = 64
QA_HTTP_ATTEMPTS = 4
MAX_EXTERNAL_COST_MICROUSD = 6_000_000
...
IMPLEMENTATION_FILES = (
    ...
    "scripts/run_longmemeval_e6_external.py",
...
def _output_process_lock(output_dir: Path) -> Iterator[None]:
    ...
    lock_path = resolved / ".e6-run.lock"
...
def _load_e1_context(args: argparse.Namespace) -> ExperimentContext:
    ...
    for question in selected:
        ...
        replay_dense_row(context, question, load_json(dense_path, sealed=True))
    return context
...
def build_e6_context(args: argparse.Namespace) -> E6Context:
    ...
    payload = {
        "artifact_type": "swarmbrain-longmemeval-e6-r0-r1-run-manifest",
        ...
        "cells": list(E6_CELLS),
        "representation": {
            "protocol": "E6/SB-HMR-v1",
            "R0": "canonical-raw-turn-keys",
            "R1": "canonical-raw-plus-one-merged-sfk-key-per-source-turn",
...
def _external_journal_cost(context: E6Context) -> tuple[int, int]:
    ...
    if total > MAX_EXTERNAL_COST_MICROUSD:
        raise ExternalE6Error("durable external call journal exceeds the $6 hard cap")
    ...
    async def reserve(self, amount: int) -> None:
        ...
        async with self.lock:
            if self.spent_microusd + self.reserved_microusd + amount > self.maximum_microusd:
                raise ExternalE6Error(
                    "E6 conservative external spend reservation would exceed the $6 cap"
                )
    ...
    async def settle(self, *, reserved: int, actual: int) -> None:
        ...
        async with self.lock:
            ...
            if self.spent_microusd > self.maximum_microusd:
                raise ExternalE6Error("E6 conservative external spend exceeded its hard cap")
...
def _extraction_summary(
    ...
    *,
    ...
    payload = {
        "artifact_type": "swarmbrain-longmemeval-e6-r1-extraction-question",
        "schema_version": ARTIFACT_SCHEMA_VERSION,
        "protocol_version": E6_RUN_PROTOCOL_VERSION,
        "run_manifest_sha256": context.manifest["artifact_sha256"],
        "question_id": question.question_id,
...
def _evaluate_r0_r1(
    ...
    *,
    ...
    return {
        RepresentationCell.RAW.value: evaluate_representation_cell(
            r0_corpus,
            cell=RepresentationCell.RAW,
            observations=(r0_raw,),
        ),
        RepresentationCell.RAW_MERGED_SFK.value: evaluate_representation_cell(
            r1_corpus,
...
def _raw_query_stage(dense: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return _stage(
        ...
        latency_microseconds=int(math.ceil(float(accounting["model_elapsed_ms"]) * 1000.0)),
...
def _rank_payload(
    ...
    *,
    ...
    payload = {
        "artifact_type": "swarmbrain-longmemeval-e6-r0-r1-ranking-question",
        "schema_version": ARTIFACT_SCHEMA_VERSION,
        "protocol_version": E6_RUN_PROTOCOL_VERSION,
        "run_manifest_sha256": context.manifest["artifact_sha256"],
        "question_id": question.question_id,
...
def _compute_pack_artifacts(
    ...
    *,
    ...
    prompt_payload = {
        "artifact_type": "swarmbrain-longmemeval-e6-r0-r1-prompts-sensitive",
        "schema_version": ARTIFACT_SCHEMA_VERSION,
        "protocol_version": E6_RUN_PROTOCOL_VERSION,
        "run_manifest_sha256": context.manifest["artifact_sha256"],
        "question_id": question.question_id,
    ...
    pack_payload = {
        "artifact_type": "swarmbrain-longmemeval-e6-r0-r1-pack-question",
        "schema_version": ARTIFACT_SCHEMA_VERSION,
        "protocol_version": E6_RUN_PROTOCOL_VERSION,
        "run_manifest_sha256": context.manifest["artifact_sha256"],
        "question_id": question.question_id,
...
def build_diagnostic_report(
    ...
    *,
    ...
    report = compile_r0_r1_diagnostic(paths)
...
def _qa_arm_order(question: SelectedQuestion) -> tuple[str, str]:
    return E6_CELLS if question.position % 2 == 0 else tuple(reversed(E6_CELLS))
...
def _validate_qa_receipt_prefix(
    ...
    *,
    ...
    expected = _qa_expected_routes(question)
...
def _qa_artifact(
    ...
    *,
    ...
    if len(receipts) != len(_qa_expected_routes(question)):
    ...
    payload = {
        "artifact_type": "swarmbrain-longmemeval-e6-r0-r1-deepseek-qa-question",
        "schema_version": ARTIFACT_SCHEMA_VERSION,
        "protocol_version": E6_RUN_PROTOCOL_VERSION,
        "run_manifest_sha256": context.manifest["artifact_sha256"],
        "question_id": question.question_id,
...
def _reconcile_qa_question_journals(
    ...
    *,
    ...
    routes = _qa_expected_routes(question)
    ...
    *,
    ...
    try:
        for completed, question in enumerate(selected, start=1):
            ...
            expected_routes = _qa_expected_routes(question)
            ...
            print(
                ...
                f"R0={qa['arms']['R0']['development_label']} "
                f"R1={qa['arms']['R1']['development_label']} "
...
def run_qa_phase(
    ...
    *,
    ...
    if diagnostic["early_stop"]["continue_to_optional_qa"] is not True:
        if _qa_durable_state_exists(context):
            raise ExternalE6Error("context early-stop is incompatible with retained QA state")
        print(
            "  qa: context gate rejected R1; no reader or judge calls executed",
            file=sys.stderr,
            flush=True,
        )
        return diagnostic
...
def build_report(context: E6Context) -> dict[str, Any]:
    ...
    for question in context.e1.selected:
        ...
        pack = load_json(e6_phase_path(context, "pack", question), sealed=True)
        prompts = load_json(e6_phase_path(context, "prompts", question), sealed=True)
        case = load_json(e6_phase_path(context, "cases", question), sealed=True)
        ...
        case_artifacts.append(case["artifact_sha256"])
        ...
        if qa_path.exists():
            ...
            for route_index, ((cell, receipt_id, role), receipt) in enumerate(
                zip(_qa_expected_routes(question), receipts, strict=True)
            ):
    ...
    early_stop = bool(diagnostic["early_stop"]["triggered"])
    ...
    payload = {
        "artifact_type": "swarmbrain-longmemeval-e6-r0-r1-development-report",
        "schema_version": ARTIFACT_SCHEMA_VERSION,
        "protocol_version": E6_RUN_PROTOCOL_VERSION,
        "run_manifest_sha256": context.manifest["artifact_sha256"],
        ...
        "external_cost": {
            ...
            "total_microusd": extraction_cost + qa_cost,
            "total_usd_upper_bound": (extraction_cost + qa_cost) / 1_000_000,
            "hard_limit_microusd": MAX_EXTERNAL_COST_MICROUSD,
            "within_hard_limit": extraction_cost + qa_cost <= MAX_EXTERNAL_COST_MICROUSD,
            "billed_cost_claimed": False,
        ...
        "decision": {
            "verdict": (
                "reject-R1-at-context-gate"
                if early_stop
                else "retain-R1-for-further-development-only"
            ),
            "context_gate_advanced_to_optional_qa": not early_stop,
            "optional_qa_executed": qa_complete,
            "eligible_for_composition": False,
            "eligible_for_serving_promotion": False,
            "eligible_for_official_gpt4o_confirmation": False,
            "required_next_if_not_rejected": (
                "scale a newly frozen paired protocol before held-out or official confirmation"
            ),
        },
        "claims": {
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "phase",
        choices=("extract", "rank", "pack", "diagnose", "qa", "report", "all"),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.sample != DEFAULT_SAMPLE or args.seed != DEFAULT_SEED:
        raise SystemExit("E6 v2 is frozen to the existing E1 10-question development sample")
    ...
    if args.extraction_concurrency != EXTRACTION_CONCURRENCY:
        raise SystemExit(f"E6 v2 extraction concurrency is frozen at {EXTRACTION_CONCURRENCY}")
    if args.limit is not None and (args.limit < 1 or args.limit > DEFAULT_SAMPLE):
        raise SystemExit(f"--limit must be between 1 and {DEFAULT_SAMPLE}")
    if args.limit is not None and args.phase in {"qa", "report"}:
        raise SystemExit("QA and final report require the complete frozen sample")
