...
class EvidenceService:
    """Any memory publisher may preserve sources and citations.

    Registration is deliberately gated only by ``memory:publish``: evidence is
    worthless unless creating it is cheaper than skipping it. Authority stays
    with the separate ``source:review`` capability, which decides how much a
    preserved source may be trusted.
    """
